...
function renderEtfLearningMatrix() {
  ...
  const card = ticker => { if (!ticker) return '<div class="etf-matrix-card is-empty"><small>Rare category — no beginner example shown</small></div>'; const f=funds[ticker]; const leveraged=f[4] !== '1×'; const isRecommended=ticker==='HDIV'; const alert=leveraged ? '<span class="matrix-alert" aria-hidden="true">⚠</span>' : ''; const star=isRecommended ? '<span class="matrix-recommendation-star" title="Featured recommendation">★</span>' : ''; const m=actualMetrics[ticker] || {}; const value=metric==='yield' ? (m.yield || f[5]) : (m.totalReturn || 'Review current fund facts'); return `<button class="etf-matrix-card${leveraged?' is-leveraged':''}${isRecommended?' is-recommended':''}" type="button" data-etf-ticker="${ticker}" aria-label="View details for ${ticker}, ${f[1]}${isRecommended?', featured recommendation':''}${leveraged?', leveraged product warning':''}">${star}${alert}<strong>${ticker}</strong><small>${f[1]}</small><span class="matrix-metric">${value}${leveraged ? ` · ${f[4]}` : ''}</span></button>`; };
