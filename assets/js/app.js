...
function renderEtfLearningMatrix() {
  ...
  const funds = {
    ...
    HDIV:['HDIV','Hamilton Enhanced Canadian Covered Call ETF','Canada','Leveraged income','1.25×','Variable monthly','Fund of funds holding multiple Canadian-focused covered-call and growth strategies; approximately 25% cash leverage','Compare CNCC, XEI, or HYLD.'],
  ...
  let metric = 'yield';
  const selectedEtfs = new Set();
  const buildComparisonUrl = () => `https://stockanalysis.com/etf/compare/${Array.from(selectedEtfs).map(ticker => `tsx:${ticker.toLowerCase()}`).join('-vs-')}/?r=MAX`;
  const updateCompareControls = () => {
    const count = document.getElementById('compareSelectedCount');
    const compare = document.getElementById('compareSelectedEtfs');
    if (count) count.textContent = `${selectedEtfs.size} selected`;
    if (compare) compare.disabled = selectedEtfs.size < 2;
  };
  const actualMetrics = {
  ...
  controls.innerHTML = '<div class="etf-matrix-toggle" role="group" aria-label="Primary ETF metric"><button type="button" class="active" data-matrix-metric="yield">Distribution yield</button><button type="button" data-matrix-metric="return">Historical total return</button></div><div class="etf-compare-actions"><button type="button" class="button compact" id="compareSelectedEtfs">Compare selected ETFs</button><span id="compareSelectedCount" class="compare-selected-count" aria-live="polite">0 selected</span></div>';
  const card = ticker => { if (!ticker) return '<div class="etf-matrix-card is-empty"><small>Rare category — no beginner example shown</small></div>'; const f=funds[ticker]; const leveraged=f[4] !== '1×'; const isRecommended=ticker==='HDIV'; const alert=leveraged ? '<span class="matrix-alert" aria-hidden="true">⚠</span>' : ''; const star=isRecommended ? '<span class="matrix-recommendation-star" title="Featured recommendation">★</span>' : ''; const m=actualMetrics[ticker] || {}; const value=metric==='yield' ? (m.yield || f[5]) : (m.totalReturn || 'Review current fund facts'); return `<article class="etf-matrix-card${leveraged?' is-leveraged':''}${isRecommended?' is-recommended':''}" data-etf-card="${ticker}">${star}${alert}<button class="etf-matrix-details" type="button" data-etf-ticker="${ticker}" aria-label="View details for ${ticker}, ${f[1]}${isRecommended?', featured recommendation':''}${leveraged?', leveraged product warning':''}"><strong>${ticker}</strong><small>${f[1]}</small><span class="matrix-metric">${value}${leveraged ? ` · ${f[4]}` : ''}</span></button><button class="etf-compare-button" type="button" data-compare-ticker="${ticker}" aria-pressed="false">Compare</button></article>`; };
  const render = () => { matrix.innerHTML=`<div class="etf-learning-grid"><div class="etf-learning-head">Geography</div><div class="etf-learning-head">Growth<br><small>No covered calls</small></div><div class="etf-learning-head">Income<br><small>Covered calls</small></div><div class="etf-learning-head">Leveraged growth</div><div class="etf-learning-head">Leveraged income</div>${rows.map(([geo,t])=>`<div class="etf-learning-row-label">${geo}</div>${t.map(card).join('')}`).join('')}</div>`; matrix.querySelectorAll('[data-etf-ticker]').forEach(b=>b.addEventListener('click',()=>openModal(b.dataset.etfTicker))); matrix.querySelectorAll('[data-compare-ticker]').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation(); const ticker=b.dataset.compareTicker; if(selectedEtfs.has(ticker)){selectedEtfs.delete(ticker);}else{selectedEtfs.add(ticker);} b.setAttribute('aria-pressed',selectedEtfs.has(ticker)); b.textContent=selectedEtfs.has(ticker)?'Selected':'Compare'; b.closest('[data-etf-card]').classList.toggle('is-selected',selectedEtfs.has(ticker)); updateCompareControls();})); matrix.querySelectorAll('[data-compare-ticker]').forEach(b=>{const selected=selectedEtfs.has(b.dataset.compareTicker); b.setAttribute('aria-pressed',selected); b.textContent=selected?'Selected':'Compare'; b.closest('[data-etf-card]').classList.toggle('is-selected',selected);}); };
  const openModal = ticker => { const f=funds[ticker]; const m=actualMetrics[ticker] || {yield:f[5],mer:'Not verified',totalReturn:'Not verified',asOf:'Not verified',source:'Not verified'}; const isHDIV=ticker==='HDIV'; document.getElementById('etfModalEyebrow').textContent=isHDIV?'★ Featured recommendation':'ETF details'; document.getElementById('etfModalCard').classList.toggle('is-recommended',isHDIV); document.getElementById('etfModalTitle').textContent=`${f[0]} — ${f[1]}`; document.getElementById('etfModalWhy').textContent=isHDIV?'Why HDIV stands out: it combines Canadian market exposure, several income and growth strategies, monthly distributions, and modest leverage in one ETF. These are structural benefits, not guarantees.':'This is a '+f[3].toLowerCase()+' example for '+f[2]+' exposure. Review the structure, risk, fees, and current fund facts before choosing.'; document.getElementById('etfModalMetrics').innerHTML=`<div><small>Distribution yield</small><strong>${m.yield}</strong></div><div><small>MER</small><strong>${m.mer}</strong></div><div><small>Historical total return</small><strong>${m.totalReturn}</strong></div><div><small>Leverage</small><strong>${f[4]}</strong></div><div><small>As of</small><strong>${m.asOf}</strong></div><div><small>Source</small><strong>${m.source}</strong></div>`; document.getElementById('etfModalComposition').innerHTML=isHDIV?'<ul><li><strong>Fund of funds:</strong> one ETF wraps several sector-focused covered-call and growth strategies.</li><li><strong>Broad Canadian focus:</strong> its sector mix is broadly similar to the S&P/TSX 60, so it participates in Canada’s growth and its setbacks.</li><li><strong>Income plus growth:</strong> covered-call strategies support monthly distributions, while growth-oriented holdings leave more room to participate when markets rise.</li><li><strong>25% cash leverage:</strong> borrowing increases exposure to about 1.25×. This can increase income and growth potential, but also increases losses, borrowing costs, and compounding drift in choppy markets.</li><li><strong>Down-market cushion:</strong> option premiums can partly offset declines. Hamilton reports HDIV outperformed the S&P/TSX 60 in calendar 2022 by 3.7%. Because HDIV launched in July 2021, this is not a full-calendar-year proof that HDIV made a positive return in 2022.</li></ul>':'<ul><li>'+f[6]+'</li><li>Check provider holdings and current distribution composition.</li></ul>'; document.getElementById('etfModalAlternatives').textContent=isHDIV?'The trade-off: distributions can change, covered calls may limit some upside, and leverage makes losses larger. Compare with unleveraged broad-market or covered-call ETFs.':f[7]; const flow=document.getElementById('etfModalFlow'); flow.hidden=!['HDIV','HYLD','EQCL','HEQL'].includes(ticker); flow.innerHTML=flow.hidden?'':'<strong>Composition flow</strong><div class="etf-flow-bar"><span>Investor capital</span><span>→ ETF</span></div><div class="etf-flow-bar"><span>Leverage and/or covered calls</span><span>→ underlying ETFs</span></div><div class="etf-flow-bar"><span>Underlying equities</span><span>→ market returns and distributions</span></div>'; document.getElementById('etfModal').hidden=false; document.querySelector('.etf-modal-close').focus(); };
  ...
  document.getElementById('compareSelectedEtfs').addEventListener('click',()=>{if(selectedEtfs.size<2)return; window.open(buildComparisonUrl(),'_blank','noopener,noreferrer');});
  updateCompareControls();
  document.querySelectorAll('[data-close-etf-modal]').forEach(e=>e.addEventListener('click',()=>{document.getElementById('etfModal').hidden=true; document.getElementById('etfModalCard').classList.remove('is-recommended');}));
