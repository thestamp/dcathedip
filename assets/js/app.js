...
function renderEtfLearningMatrix() {
  ...
  const card = ticker => { if (!ticker) return '<div class="etf-matrix-card is-empty"><small>Rare category — no beginner example shown</small></div>'; const f=funds[ticker]; const leveraged=f[4] !== '1×'; const drop=shock ? `<span class="matrix-metric">Shock: ${(shock*(leveraged?1.25:1)).toFixed(1)}%</span>` : ''; const alert=leveraged ? '<span class="matrix-alert" aria-hidden="true">⚠</span>' : ''; const m=actualMetrics[ticker] || {}; const value=metric==='yield' ? (m.yield || f[5]) : (m.totalReturn || 'Review current fund facts'); return `<button class="etf-matrix-card${leveraged?' is-leveraged':''}" type="button" data-etf-ticker="${ticker}" aria-label="View details for ${ticker}, ${f[1]}${leveraged?', leveraged product warning':''}">${alert}<strong>${ticker}</strong><small>${f[1]}</small><span class="matrix-metric">${value} · ${f[4]}</span>${drop}</button>`; };
